...
type connectSpannerTestCase struct {
	name                   string
	dbName                 string
...
func TestConnectToSpanner(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			connectedDB, err := ConnectToSpanner(ctx, spannerContainer.projectID, spannerContainer.instanceID, tt.dbName, spannerContainer.opts...)
			...
			if connectedDB == nil {
				t.Fatalf("connectedDB should not be nil for db %s", tt.dbName)
			}
			defer func() {
				if err := connectedDB.Close(); err != nil {
					t.Errorf("connectedDB.Close failed for %s: %v", tt.dbName, err)
				}
			}()
			...
			if tt.expectMigrateUpError {
				if err == nil {
					t.Errorf("connectedDB.MigrateUp should have failed for db %s, but got nil error", tt.dbName)
				}
			} else {
				if err != nil {
					t.Errorf("connectedDB.MigrateUp should succeed for db %s: %v", tt.dbName, err)
				}
			}
			...
			if !tt.expectMigrateUpError {
				...
				if tt.expectMigrateDownError {
					if err == nil {
						t.Errorf("connectedDB.MigrateDown should have failed for db %s, but got nil error", tt.dbName)
					}
				} else {
					if err != nil {
						t.Errorf("connectedDB.MigrateDown should succeed for db %s: %v", tt.dbName, err)
					}
				}
			}
		})
